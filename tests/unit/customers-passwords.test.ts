...
describe('hashPassword', () => {
  test('produces a self-describing verifier that records its own cost parameters', () => {
  ...
  test('never contains the password', () => {
...
describe('verifyPassword', () => {
  test('accepts the correct password and rejects a wrong one', () => {
    ...
    assert.equal(verifyPassword('wrong', verifier), false);
  });

  test('rejects a near-miss, including case and whitespace differences', () => {
    const verifier = hashPassword(PASSWORD);
    const nearMisses = [PASSWORD.toUpperCase(), ` ${PASSWORD}`, `${PASSWORD} `, PASSWORD.slice(0, -1)];

    for (const candidate of nearMisses) {
      assert.equal(verifyPassword(candidate, verifier), false, `accepted near-miss "${candidate}"`);
    }
  });

  test('denies rather than throws on a malformed verifier', () => {
    ...
    for (const malformed of ['', 'not-a-verifier', 'scrypt$1$2$3', 'scrypt$a$b$c$d$e', 'bcrypt$1$8$1$aa$bb']) {
      assert.equal(verifyPassword(PASSWORD, malformed), false, `accepted malformed verifier "${malformed}"`);
    }
  });

  test('denies a verifier whose cost would exhaust memory, instead of attempting it', () => {
    ...
    assert.equal(verifyPassword(PASSWORD, absurd), false);
  });
});

describe('the dummy verifier', () => {
  test('is a real verifier, so an unknown email costs the same work as a wrong password', () => {
    assert.match(DUMMY_VERIFIER, /^scrypt\$/);
    assert.equal(verifyPassword(PASSWORD, DUMMY_VERIFIER), false);
  });
});
