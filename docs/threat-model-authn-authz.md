...
- **Password guessing.** **Decision:** per-account lockout after repeated failures, using a fixed window. The failure count is incremented atomically in SQL, so a concurrent burst of guesses cannot each overwrite the same stale count and evade the lock (a race the async-hash rewrite introduced and Phase 12 review caught). Not IP-based: this binds to loopback and IP throttling would be theatre here. **Residual:** the lockout is itself a denial vector — anyone who knows a victim's email can submit five wrong passwords and lock that account for the window. Pre-existing and not changed by the Phase 12 work; accepted on the same loopback rationale as the distinct-address residual, and a per-source network control upstream is where it would be closed.
